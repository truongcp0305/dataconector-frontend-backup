...
/**
 * Lớp xử lý date cell renderer
 */
export default class DateCellRenderer extends BaseCellRenderer {
    constructor() {
        super();
        this.placeHolder = '';
        this.rightIcon =
    ...
    renderGroupRow() {
        let patternDateStr = this.control.controlProperties.formatDate.value
            ? this.control.controlProperties.formatDate.value
            : 'DD/MM/YYYY';
        this.placeHolder = patternDateStr;
        let valueFormatted = this.control.getDateChange(this.value);
    ...
    checkDateValid() {
        ...
        if (valueFormatted == false) {
            if (this.value != '') {
                check = true;
                this.control.valueValidation['DateValid'][this.rowId] = {
                    isValid: true,
                    msg: SYMPER_APP.$t('v2.doc.incorrectDateFormatAllowed'),
                };
            }
        } else {
            this.control.valueValidation['DateValid'][this.rowId] = {
