<template>
    <div class="h-100 w-100">
        <v-btn icon x-small @click="handleFocus"> </v-btn>
        <symper-bpmn
    ...
    methods: {
        ...
        plotHeatmap() {
            ...
            if (currentTrackingProcess) {
                ...
                allNodes.forEach(function (e) {
                    if (e.$type == 'bpmn:UserTask') {
                        currentTrackingProcess.forEach(function (k) {
                            if (k.act_id_ == e.id) {
                                let sum =
                                    parseInt(k.count_end) +
                                    parseInt(k.count_running);
                ...
                var canvas =
                    document.getElementsByClassName('heatmap-canvas')[0];
                setTimeout(
                    (self) => {
                        self.focusCanvas();
                        canvas.height = 500;
                                    ...
                                    actionBtns: [
                                        {
                                            ...
                                            icon: 'mdi-send-check',
                                            action: (close) => {
                                                let symperBpmnHeatMap =
                                                    self.$refs
                                                        .symperBpmnHeatMap;
                                                let errEl =
                                                    symperBpmnHeatMap.getElData(
                                                        err.activityId
                                                    );
                                                symperBpmnHeatMap.bpmnModeler
                                                    .get('selection')
                                                    .select(errEl);
        ...
        getModelData() {
            ...
            let vizBizEl = this.$refs.symperBpmnHeatMap.updateElementProperties(
                modelAttr.id,
                {
        ...
        getModelDataForSymperService() {
            ...
            for (let el of allVizEls) {
                if (el.businessObject.$type == 'bpmn:Task') {
                    this.$refs.symperBpmnHeatMap.changeTaskNodeToUserTaskNode(
                        el.id
                    );
                }
            }
        ...
        setFlowsOrderForGateway(nodeData) {
            let outFlows = [];
            let bizNodeData = this.$refs.symperBpmnHeatMap.getElData(
                nodeData.id
            ).businessObject;
            let diOutFlowsMap = {};
        ...
        async applySavedData(idProcess) {
            try {
                ...
                let afterRender =
                    await this.$refs.symperBpmnHeatMap.renderFromXML(xml);
