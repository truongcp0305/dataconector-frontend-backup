<template>
    <div class="view-details-all-app h-100" style="display: flex">
        <div class="header-view-details-all-app">
            ...
            <div style="width: 500px; display: flex">
                <v-btn
                    ...
                    @click="createTask"
                >
                    <v-icon left dark style="padding-right: 8px"
                        >mdi-plus</v-icon
                    >
        ...
        <DialogCreateTask
            ...
            @close-dialog="showCreateTask = false"
        />
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getActiveapps() {
            ...
            let appIds = [];
            if(this.$store.state.app.userOperations){
                appIds = Object.keys(this.$store.state.app.userOperations.application_definition);
            }
            appIds = appIds.join(',');
            ...
            this.listType.forEach(function (k) {
                ...
                if (data.hasOwnProperty(k)) {
                    data[k].forEach(function (e) {
                        ...
                        self.mapIdApp[idApp][k].push(k + ':' + e.id);
    ...
    mounted() {
        ...
        if (
            !Object.keys(this.listApp[0]).length &&
            !Object.keys(this.listApp[1]).length
        ) {
            setTimeout(() => {
                this.getActiveapps();
            }, 1000);
