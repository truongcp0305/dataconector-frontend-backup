    ...
    <div class="h-100 w-100">
        <div>
            <h4 class="fw-430 header-title">
                ...
                <span class="ml-0 fs-15"> {{$t('v2.account.showDetail')}} </span>
            </h4>
        </div>
        <v-stepper class="w-100 d-flex stepper-create-user">
            <div class="w-100 ml-1 mt-2 fs-15" v-if="!isViewUserRole">
                <h4 style="font-weight: 430 !important">
                    {{ $t('user.general.personalInfo.title') }}
                </h4>
                <v-row class="mt-1">
                    <!-- thong tin -->
                    <v-col cols="8">
                        ...
                        <v-row class="ml-1" style="margin-top: -15px">
                            ...
                            <v-col cols="6">
                                <span
                                    style="color: green"
                                    v-if="detailInfo.status == 'Đang hoạt động'"
                                    class="fs-13"
                                >
                                {{$t('v2.account.active')}}
                                </span>
                                <span
                                    style="color: red"
                                    class="fs-13"
                                    v-else-if="detailInfo.status == 'Mới tạo'"
                                >
                                {{$t('v2.account.newCreate')}}
                                </span>
                                <span
                                    style="color: orange"
                                    class="fs-13"
                ...
                <h4 class="mb-2 fs-15 fw-430">{{$t('v2.account.permissions')}}</h4>
...
export default {
    ...
    props: ['detailInfo', 'changeDetail', 'showDetailView', 'showUpdateBtn'],
    computed: {
        sapp() {
