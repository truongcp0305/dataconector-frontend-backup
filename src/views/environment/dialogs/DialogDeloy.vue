<template>
    <div class="dialog-deploy">
        <v-dialog v-model="showDialog" persistent max-width="400">
            <v-card>
                <v-card-title class="fs-15"> Deploy version </v-card-title>
                <v-card-text>
                    <div
                        class="content-deploy-dialog d-flex flex-column ml-2 fs-13"
                    >
                        <div class="fs-13 mb-2 mt-2">
                            {{$t('v2.environment.choseEnvDeploy')}}
                        </div>
                        <v-autocomplete
                            v-model="envId"
                            :items="allEnv"
                        ...
                        ></v-autocomplete>
                        <div class="text-wrap">
                            {{$t('v2.environment.pressToDeployVersion')}}
                        </div>
                    </div>
                </v-card-text>
                <v-card-actions>
                    ...
                    <v-btn color="green darken-1" text @click="deployVersion">
                        {{$t('v2.environment.deploy')}}
                    </v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
