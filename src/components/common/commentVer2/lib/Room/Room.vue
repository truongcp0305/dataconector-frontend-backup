<template>
    ...
    >
        ...
        <room-header
            ...
            @hide-comment-panel="$emit('hide-comment-panel')"
        ...
        <div
            ...
            :style="{ 'margin-top': showRoomHeader ? '37px' : '5px' }"
        >
            ...
            <div class="vac-messages-container h-100">
                <div :class="{ 'vac-messages-hidden': loadingMessages, 'h-100': true }">
                    <transition name="vac-fade-message">
                            <div v-if="showNoMessages" class="vac-text-started">
                                <slot name="messages-empty">
                                    {{ $t('comment.noComment') }}.
                                    <span
                                        @click="getTextareaRef().focus()"
                                        style="
                                            color: #f48634;
                                            font-weight: 500;
                                            cursor: pointer;
                    ...
                    <transition name="vac-fade-message">
                        <infinite-loading
                            ...
                            :class="{
                                'vac-infinite-loading': !messagesLoaded,
                                xxxxxxxxxxxx: true,
                            }"
                    ...
                    >
                        ...
                        >
                            ...
                            <template
                                v-if="
                                    !isNotification ||
                                    (isNotification && commentSubId == null)
                                "
                            >
                                <span
                                    class="see__more__comment__child"
                                    v-if="m.isShowChild && m.isSeeMoreChild"
                                    @click="
                                        seeMoreCommentChild(
                                            m.uuid,
                                            i,
                            ...
                            <template v-else>
                                ...
                                <span
                                    ...
                                    v-if="
                                        ...
                                        nextSubId != '' &&
                                        commentSubId != null &&
                                        m.uuid == lastSubIdSplice
                                    "
                                    @click="getCommentBehind(commentId)"
                                >
                    ...
                    <p
                        class="see__more__comment"
                        v-if="isNotification && nextId != ''"
                        @click="getCommentBehind(null)"
                    >
                        {{ $t('comment.seeMoreComments') }}
                    </p>
                </div>
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <div v-else class="vac-svg-button" @click="">
                ...
                <textarea
                    ref="roomTextarea"
                    :placeholder="$t('comment.commentHere')"
                    class="vac-textarea"
                    :class="{
...
export default {
    ...
    props: {
        lastSubIdSplice: { type: String, default: '' },
        showRoomHeader: { type: Boolean, default: false },
        scrollBottom: { type: Boolean, default: false },
        nextId: { type: String, default: '' },
        lastSubId: { type: String, default: '' },
        nextSubId: { type: String, default: '' },
        ...
        isFocusComment: { type: Boolean, default: true },
    },
    ...
    created() {
        if (this.isNotification) {
            if (this.$route.params.extraData.parentId != null) {
                ...
                this.commentSubId = this.$route.params.extraData.commentId;
            } else {
                this.commentId = this.$route.params.extraData.commentId;
            }
        }
        this.infiniteId = util.str.randomString(6);
    },
    ...
    watch: {
        ...
        message(val) {
            ...
            this.$emit('typing-message-1', val);
        },
        loadingMessages(val) {
            if (val) {
    ...
    methods: {
        ...
        seeMoreCommentChild(uuid, index, childPage) {
            const self = this;
            ...
            self.timeOut = setTimeout(() => {
                ...
                self.$emit('display-comment-child', data);
            }, 600);
        },
        seeMoreCommentChildTag() {
            const self = this;

            clearTimeout(self.timeOut);
            self.timeOut = setTimeout(() => {
                self.$emit('display-comment-child-tag');
            }, 600);
        },
        getCommentBehind(parentId) {
            const self = this;
            clearTimeout(self.timeOut);
            self.timeOut = setTimeout(() => {
                self.$emit('get-comment-behind', parentId);
            }, 400);
        },
        displayCommentChild(uuid) {
            const self = this;
            let messagesLength = self.messages.length;
            for (let i = 0; i < messagesLength; i++) {
                if (self.messages[i].uuid == uuid) {
                    self.messages[i].isShowChild = true;
        ...
        touchStart(touchEvent) {
            ...
            if (touchEvent.changedTouches.length === 1) {
                ...
                addEventListener(
                    'touchend',
                    (touchEvent) =>
                        this.touchEnd(touchEvent, posXStart, posYStart),
                    { once: true },
                );
            }
        },
        ...
        onMessageAdded({ message, index, ref }) {
            ...
            setTimeout(() => {
                if (
                    this.getBottomScroll(this.$refs.scrollContainer) <
                    autoScrollOffset
                ) {
                    this.scrollToBottom();
                } else {
        ...
        scrollToMessage() {
            let commentTag = this.commentId
            let com  = this.messages.filter((m)=>{ 
                if(m.uuid == commentTag){
                    return m
                }
            })
            if(com){
                document.getElementById(com[0]._id).scrollIntoView(true);
            }
        },
        onContainerScroll(e) {
        ...
        updateShowUsersTag(query) {
            let k = filteredItems(
                ...
                query,
                false,
            ).filter((user) => user._id !== String(this.currentUserId));
            this.filteredUsersTag = k.slice(0, 3);
        },
        ...
        sendMessage() {
            ...
            if (message.length >= 1000) {
                this.callNotify('error', this.$t('comment.textMaxLength'));
                this.getTextareaRef().focus();
                return;
            }
            ...
            let k = [];
            if (this.selectedUsersTag.length > 0) {
            ...
            if (files != null) {
                ...
                if (totalSize >= 41943040) {
                    this.callNotify('error', this.$t('comment.fileMaxSize'));
                    return;
                }
            }
            if (this.editedMessage._id) {
                if (
                    ...
                    this.$emit('edit-message', {
                        messageId: this.editedMessage.uuid,
                        newContent: message,
                        files: files,
        ...
        messageActionHandler({ action, message }) {
            switch (action.name) {
                case 'replyMessage':
                    ...
                    for (let i = 0; i < messagesLength; i++) {
                        if (
                            this.messages[i]._id == message._id &&
                            this.messages[i].isShowChild == false
                        ) {
                            this.displayCommentChild(message.uuid);
                        }
                    }
        ...
        replyMessage(message) {
            ...
            this.selectedUsersTag = [];
            let k = {};
            k._id = message.senderId;
            k.avatar = message.avatar;
            k.username = message.username;
            k.status = {};
            this.message = '@' + message.username;
            this.selectedUsersTag.push(k);
        },
        editMessage(message) {
            this.resetMessage();
