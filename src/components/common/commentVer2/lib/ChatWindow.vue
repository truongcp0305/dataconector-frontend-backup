...
export default {
    ...
    props: {
        ...
        messageActions: {
            type: Array,
            default: () => [
                { name: 'replyMessage', title: SYMPER_APP.$t('v2.comment.reply') },
                { name: 'editMessage', title: SYMPER_APP.$t('v2.comment.editMessage'), onlyMe: true },
                {
                    name: 'deleteMessage',
                    title: SYMPER_APP.$t('v2.comment.deleteMessage'),
                    onlyMe: true,
                },
            ],
        },
        ...
        rooms: {
            ...
            handler(newVal, oldVal) {
                ...
                if (
                    ...
                    if (this.roomId) {
                        const room =
                            newVal.find((r) => r.roomId === this.roomId) || {};
                        this.fetchRoom({ room });
    ...
    methods: {
        ...
        displayCommentChild(data) {
            this.$emit('display-comment-child', data);
        },
        displayCommentChildTag() {
            this.$emit('display-comment-child-tag');
        },
        getCommentBehind(parentId) {
            this.$emit('get-comment-behind', parentId);
        },
        updateResponsive() {
